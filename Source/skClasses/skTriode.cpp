...
SKTriode::SKTriode(int numStages)
{
    params.dcBlockFreq = 10.0;

    params.interStageFilterFreq = 3000.0;
    ...
    for (int i = 0; i < params.numStages; i++)
    {
        params.stageGains.push_back(1.0);

        filters[i].reset(new skFilters(lowpass));

        zvaFilters[i].reset(new ZVAFilter());
    }
}

void SKTriode::setGain(std::vector<double> gains)
{
    for (int i = 0; i < gains.size(); i++)
    {
        params.stageGains[i] = gains[i];
    }
}
...
void SKTriode::setCompressionStyle(bool onOff)
{
    params.compState = onOff;
}

void SKTriode::reset(double Fs)
{
    ZVAFilterParameters zvaParams = zvaFilters[0]->getParameters();
    zvaParams.filterAlgorithm = vaFilterAlgorithm::kLPF1;
    zvaParams.fc = 5.0;

    for (int i = 0; i < params.numStages; i++)
    {
        filters[i]->calc(params.interStageFilterFreq, 1.0, 0.707, Fs);
        zvaFilters[i]->reset(Fs);
        zvaFilters[i]->setParameters(zvaParams);
    }
}

juce::dsp::AudioBlock<float> SKTriode::clipBlock(juce::dsp::AudioBlock<float> block, int numChannels)
{
    for (int stage = 0; stage < params.numStages; stage++)
    {
        for (int sample = 0; sample < block.getNumSamples(); sample++)
        {
            for (int channel = 0; channel < numChannels; channel++)
            {
                ...
                chunk[sample] *= params.stageGains[stage];
                ...
                if (params.compState)
                {
                    ...
                    dcOffset = zvaFilters[stage]->processAudioSample(chunk[sample]);
                ...
                chunk[sample] = filters[stage]->process(chunk[sample]);
